// Seed exercise catalog. Taxonomy per PRD §5 / CLAUDE.md §6.5: primary +
// secondary required; tertiary ONLY where it materially affects fatigue
// accounting (e.g. triceps under heavy pressing are secondary, not tertiary,
// so the engine won't program triceps isolation the same day).
//
// `equipment` lists EVERY tag required to perform the movement; an exercise
// is available only when the active gym profile covers all of them.

import type { Exercise } from '@/domain/types';

export const EXERCISE_CATALOG: readonly Exercise[] = [
  // ── Push — compounds ──────────────────────────────────────────────
  {
    id: 'barbell-bench-press',
    name: 'Barbell Bench Press',
    exerciseClass: 'compound',
    equipment: ['barbell', 'bench'],
    primaryMuscles: ['chest'],
    secondaryMuscles: ['triceps', 'shoulders'],
  },
  {
    id: 'incline-barbell-bench-press',
    name: 'Incline Barbell Bench Press',
    exerciseClass: 'compound',
    equipment: ['barbell', 'bench'],
    primaryMuscles: ['chest', 'shoulders'],
    secondaryMuscles: ['triceps'],
  },
  {
    id: 'dumbbell-bench-press',
    name: 'Dumbbell Bench Press',
    exerciseClass: 'compound',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['chest'],
    secondaryMuscles: ['triceps', 'shoulders'],
  },
  {
    id: 'incline-dumbbell-press',
    name: 'Incline Dumbbell Press',
    exerciseClass: 'compound',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['chest', 'shoulders'],
    secondaryMuscles: ['triceps'],
  },
  {
    id: 'overhead-press',
    name: 'Overhead Press',
    exerciseClass: 'compound',
    equipment: ['barbell'],
    primaryMuscles: ['shoulders'],
    secondaryMuscles: ['triceps'],
    tertiaryMuscles: ['core'],
  },
  {
    id: 'seated-dumbbell-shoulder-press',
    name: 'Seated Dumbbell Shoulder Press',
    exerciseClass: 'compound',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['shoulders'],
    secondaryMuscles: ['triceps'],
  },
  {
    id: 'push-up',
    name: 'Push-Up',
    exerciseClass: 'compound',
    equipment: ['bodyweight'],
    primaryMuscles: ['chest'],
    secondaryMuscles: ['triceps', 'shoulders'],
    tertiaryMuscles: ['core'],
  },
  {
    id: 'machine-chest-press',
    name: 'Machine Chest Press',
    exerciseClass: 'compound',
    equipment: ['machine'],
    primaryMuscles: ['chest'],
    secondaryMuscles: ['triceps', 'shoulders'],
  },

  // ── Pull — compounds ──────────────────────────────────────────────
  {
    id: 'deadlift',
    name: 'Deadlift',
    exerciseClass: 'compound',
    equipment: ['barbell'],
    primaryMuscles: ['hamstrings', 'glutes', 'back'],
    secondaryMuscles: ['quads', 'forearms', 'core'],
  },
  {
    id: 'romanian-deadlift',
    name: 'Romanian Deadlift',
    exerciseClass: 'compound',
    equipment: ['barbell'],
    primaryMuscles: ['hamstrings', 'glutes'],
    secondaryMuscles: ['back', 'forearms'],
  },
  {
    id: 'barbell-row',
    name: 'Barbell Row',
    exerciseClass: 'compound',
    equipment: ['barbell'],
    primaryMuscles: ['back'],
    secondaryMuscles: ['biceps'],
    tertiaryMuscles: ['forearms', 'core'],
  },
  {
    id: 'dumbbell-row',
    name: 'One-Arm Dumbbell Row',
    exerciseClass: 'compound',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['back'],
    secondaryMuscles: ['biceps'],
    tertiaryMuscles: ['forearms'],
  },
  {
    id: 'pull-up',
    name: 'Pull-Up',
    exerciseClass: 'compound',
    equipment: ['bodyweight', 'pullupBar'],
    primaryMuscles: ['back'],
    secondaryMuscles: ['biceps'],
    tertiaryMuscles: ['forearms', 'core'],
  },
  {
    id: 'lat-pulldown',
    name: 'Lat Pulldown',
    exerciseClass: 'compound',
    equipment: ['cable'],
    primaryMuscles: ['back'],
    secondaryMuscles: ['biceps'],
  },
  {
    id: 'seated-cable-row',
    name: 'Seated Cable Row',
    exerciseClass: 'compound',
    equipment: ['cable'],
    primaryMuscles: ['back'],
    secondaryMuscles: ['biceps'],
    tertiaryMuscles: ['forearms'],
  },

  // ── Legs — compounds ──────────────────────────────────────────────
  {
    id: 'barbell-back-squat',
    name: 'Barbell Back Squat',
    exerciseClass: 'compound',
    equipment: ['barbell', 'rack'],
    primaryMuscles: ['quads', 'glutes'],
    secondaryMuscles: ['core'],
    tertiaryMuscles: ['hamstrings'],
  },
  {
    id: 'front-squat',
    name: 'Front Squat',
    exerciseClass: 'compound',
    equipment: ['barbell', 'rack'],
    primaryMuscles: ['quads'],
    secondaryMuscles: ['glutes', 'core'],
  },
  {
    id: 'goblet-squat',
    name: 'Goblet Squat',
    exerciseClass: 'compound',
    equipment: ['dumbbell'],
    primaryMuscles: ['quads', 'glutes'],
    secondaryMuscles: ['core'],
  },
  {
    id: 'leg-press',
    name: 'Leg Press',
    exerciseClass: 'compound',
    equipment: ['machine'],
    primaryMuscles: ['quads', 'glutes'],
    secondaryMuscles: ['hamstrings'],
  },
  {
    id: 'bulgarian-split-squat',
    name: 'Bulgarian Split Squat',
    exerciseClass: 'compound',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['quads', 'glutes'],
    secondaryMuscles: ['hamstrings', 'core'],
  },
  {
    id: 'barbell-hip-thrust',
    name: 'Barbell Hip Thrust',
    exerciseClass: 'compound',
    equipment: ['barbell', 'bench'],
    primaryMuscles: ['glutes'],
    secondaryMuscles: ['hamstrings'],
  },

  // ── Isolations ────────────────────────────────────────────────────
  {
    id: 'dumbbell-curl',
    name: 'Dumbbell Curl',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['biceps'],
    secondaryMuscles: ['forearms'],
  },
  {
    id: 'barbell-curl',
    name: 'Barbell Curl',
    exerciseClass: 'isolation',
    equipment: ['barbell'],
    primaryMuscles: ['biceps'],
    secondaryMuscles: ['forearms'],
  },
  {
    id: 'hammer-curl',
    name: 'Hammer Curl',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['biceps', 'forearms'],
    secondaryMuscles: [],
  },
  {
    id: 'triceps-pushdown',
    name: 'Triceps Pushdown',
    exerciseClass: 'isolation',
    equipment: ['cable'],
    primaryMuscles: ['triceps'],
    secondaryMuscles: [],
  },
  {
    id: 'overhead-triceps-extension',
    name: 'Overhead Triceps Extension',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['triceps'],
    secondaryMuscles: [],
  },
  {
    id: 'skull-crusher',
    name: 'Skull Crusher',
    exerciseClass: 'isolation',
    equipment: ['barbell', 'bench'],
    primaryMuscles: ['triceps'],
    secondaryMuscles: [],
  },
  {
    id: 'lateral-raise',
    name: 'Lateral Raise',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['shoulders'],
    secondaryMuscles: [],
  },
  {
    id: 'rear-delt-fly',
    name: 'Rear Delt Fly',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['shoulders'],
    secondaryMuscles: ['back'],
  },
  {
    id: 'face-pull',
    name: 'Face Pull',
    exerciseClass: 'isolation',
    equipment: ['cable'],
    primaryMuscles: ['shoulders'],
    secondaryMuscles: ['back'],
  },
  {
    id: 'dumbbell-chest-fly',
    name: 'Dumbbell Chest Fly',
    exerciseClass: 'isolation',
    equipment: ['dumbbell', 'bench'],
    primaryMuscles: ['chest'],
    secondaryMuscles: [],
  },
  {
    id: 'cable-fly',
    name: 'Cable Fly',
    exerciseClass: 'isolation',
    equipment: ['cable'],
    primaryMuscles: ['chest'],
    secondaryMuscles: [],
  },
  {
    id: 'leg-extension',
    name: 'Leg Extension',
    exerciseClass: 'isolation',
    equipment: ['machine'],
    primaryMuscles: ['quads'],
    secondaryMuscles: [],
  },
  {
    id: 'leg-curl',
    name: 'Leg Curl',
    exerciseClass: 'isolation',
    equipment: ['machine'],
    primaryMuscles: ['hamstrings'],
    secondaryMuscles: [],
  },
  {
    id: 'dumbbell-calf-raise',
    name: 'Dumbbell Calf Raise',
    exerciseClass: 'isolation',
    equipment: ['dumbbell'],
    primaryMuscles: ['calves'],
    secondaryMuscles: [],
  },
  {
    id: 'machine-calf-raise',
    name: 'Machine Calf Raise',
    exerciseClass: 'isolation',
    equipment: ['machine'],
    primaryMuscles: ['calves'],
    secondaryMuscles: [],
  },
  {
    id: 'cable-crunch',
    name: 'Cable Crunch',
    exerciseClass: 'isolation',
    equipment: ['cable'],
    primaryMuscles: ['core'],
    secondaryMuscles: [],
  },
  {
    id: 'hanging-leg-raise',
    name: 'Hanging Leg Raise',
    exerciseClass: 'isolation',
    equipment: ['bodyweight', 'pullupBar'],
    primaryMuscles: ['core'],
    secondaryMuscles: ['forearms'],
  },
] as const;

/** Lookup by id; returns undefined for unknown ids — caller decides severity. */
export function getExerciseById(id: string): Exercise | undefined {
  return EXERCISE_CATALOG.find((exercise) => exercise.id === id);
}
